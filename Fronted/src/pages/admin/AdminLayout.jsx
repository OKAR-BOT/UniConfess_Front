...
function AdminSidebar() {
  ...
  return (
    <aside className="sidebar-admin relative flex w-56 shrink-0 flex-col gap-1 overflow-hidden px-3 py-6 max-lg:hidden">
      <div className="sidebar-admin-glow pointer-events-none absolute inset-0" />
      ...
      {SIDEBAR_LINKS.map((link) => (
        <NavLink
          ...
          className={`relative z-10 flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition-all duration-200 ${
  ...
  return (
    <div className="flex min-h-[calc(100vh-4.25rem)]" style={{ background: 'var(--color-bg)' }}>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="flex-1 overflow-auto admin-content">
          <Outlet />
        </div>
      </div>
    </div>
